...
fun viewComments(episodeId: Int) {
    ...
    Box(
        ...
        modifier = Modifier
            .fillMaxSize()
            .padding(start = 16.dp) // Adding left padding
    ) {
        if (commentsWithTitles.isNullOrEmpty()) {
            Text(
                text = "No comments to be published yet",
                    ...
                    itemsIndexed(commentsForEpisode) { commentIndex, comment ->
                        Row(
                            ...
                            Column(
                                ...
                                Button(
                                    onClick = {
                                        publishComment(episodeId, comment, context)
                                        fetchUnapprovedComments(episodeId) { fetchedCommentsWithTitles ->
                                            commentsWithTitles = fetchedCommentsWithTitles
                                        }
                                    },
                                ...
                                Button(
                                    onClick = {
                                        deleteComment(episodeId, comment, context)
                                        fetchUnapprovedComments(episodeId) { fetchedCommentsWithTitles ->
                                            commentsWithTitles = fetchedCommentsWithTitles
                                        }
                                    },
